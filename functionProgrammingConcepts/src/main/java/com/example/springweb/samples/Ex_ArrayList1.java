...
public class Ex_ArrayList1 {
	...
	public static void main(String[] args) {
		...
		String l2 = s1.stream().max((a1, a2) -> (a1.age > a2.age) ? 1 : -1).map(b -> b.studentName).get();
		...
		//Map<String,Long> m=IntStream(0,name.length())
		/*
		 * for (int i = 0; i < name1.length; i++) { int count = 0; for (int j = 0; j <
		 * name1.length; j++) { if (name1[i] == name1[j]) { count++; } } st.add(name1[i]
		 * + " - " + count); } for(String str:st) { System.out.println(str); }
		 */
		...
		while (I.hasNext()) {
			Student s = I.next();
			System.out.println(s.studentName + " with roll no " + s.rollNo + " is of Age " + s.age);
		}
		...
		System.out.println(l.stream().collect(Collectors.groupingBy(String::length,
				Collectors.filtering(s -> !s.contains("V"), Collectors.toList()))));
